...
// ── Konfigirasyon VAPID pou Push Notifications ─────
webpush.setVapidDetails(
  process.env.VAPID_EMAIL || 'mailto:admin@ayitimarket.ht',
...
app.post('/api/messages', async (req, res) => {
  ...
  // ── Voye Push Notification bay destinatè a ────────
  ...
  const senderName = sender?.name || 'Yon moun';
  const preview = body?.trim()
    ? (body.trim().length > 60 ? body.trim().slice(0, 57) + '…' : body.trim())
    : (file_type?.startsWith('image') ? '📷 Foto' : '📎 Fichye');
...
// Frontend rele sa pou konnen kle piblik VAPID la
app.get('/api/vapid-public-key', (req, res) => {
  if (!process.env.VAPID_PUBLIC_KEY) {
    return res.status(503).json({ error: 'Push notifications pa konfigire' });
  }
  res.json({ key: process.env.VAPID_PUBLIC_KEY });
});

// ── POST /api/push-subscribe ───────────────────────
// Itilizatè voye abònman push li (endpoint + kle) pou nou sove l
app.post('/api/push-subscribe', async (req, res) => {
  ...
  if (!subscription?.endpoint) {
    return res.status(400).json({ error: 'Abònman invalid' });
  }

  // Sove / mete ajou abònman an nan tablo push_subscriptions
  const { error } = await supabase
...
// Itilizatè retire abònman (si yo derefize notifikasyon)
...
// ── Fonksyon entèn: voye notifikasyon push a yon itilizatè ──
async function sendPushToUser(recipientId, payload) {
  if (!process.env.VAPID_PUBLIC_KEY) return; // Push pa konfigire, sote
  ...
  await Promise.allSettled(
    subs.map(async (sub) => {
      try {
        ...
        // Si abònman ekspire oswa invalid, efase l
