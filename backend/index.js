...
async function applyBoost({ listingId, sellerId, tier, days, priceHtg, notes }) {
...
app.get('/api/boost/moncash/return', async (req, res) => {
  ...
  try {
    ...
    const paymentId = (orderId || tx.reference || '').replace('boost-', '');
...
app.put('/api/admin/boost-payments/:id/approve', async (req, res) => {
  ...
  await applyBoost({
    listingId: payment.listing_id, sellerId: payment.seller_id,
    tier: payment.tier, days: payment.days, priceHtg: payment.price_htg,
    notes: `NatCash #${payment.natcash_reference} (verifye pa ${user.email})`
  });
...
// ── SEO: paj dedye pou chak kategori (/categorie/elektronik) ───────
...
app.get('/categorie/:slug', async (req, res) => {
  ...
  const canonicalUrl = `${frontendUrl}/categorie/${slug}`;
...
app.get('/sitemap.xml', async (req, res) => {
  ...
  const categoryUrls = Object.keys(CATEGORY_SLUGS).map(slug =>
    `  <url><loc>${frontendUrl}/categorie/${slug}</loc></url>`
  ).join('\n');
