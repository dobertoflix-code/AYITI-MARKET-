...
// ════════════════════════════════════════════════════
// ESTATISTIK VANDÈ — listing_views
// ════════════════════════════════════════════════════

// ── POST /api/listings/:id/view ────────────────────
// Anrejistre yon vizit sou yon anons (piblik, san otantifikasyon obligatwa)
// Apèl: fetch(`${API_BASE}/api/listings/${id}/view`, { method:'POST' })
app.post('/api/listings/:id/view', async (req, res) => {
  const listingId = req.params.id;
  if (!listingId) return res.status(400).json({ error: 'listing_id obligatwa' });

  // Verifye anons la egziste e aktif
  ...
  // Optionèl: jwenn user_id si konekte
  let viewerId = null;
...
// Estatistik pou vandè konekte: total views, pa anons, pa jou (28 dènye jou)
app.get('/api/seller/stats', async (req, res) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ error: 'Pa otorize' });
  const token = authHeader.replace('Bearer ', '');
  const { data: { user }, error: authError } = await supabase.auth.getUser(token);
  if (authError || !user) return res.status(401).json({ error: 'Sesyon ekspire' });
  ...
  // Views 28 dènye jou
  const { count: views28 } = await supabase
    .from('listing_views')
    .select('id', { count: 'exact', head: true })
    .eq('seller_id', user.id)
    .gte('viewed_at', since28);

  // Views pa anons (top 10)
  ...
  const listingMap = {};
  (perListing || []).forEach(v => {
    const key = v.listing_id;
    if (!listingMap[key]) listingMap[key] = { listing_id: key, title: v.listings?.title || '—', views: 0 };
    listingMap[key].views++;
  });
  const topListings = Object.values(listingMap)
    .sort((a, b) => b.views - a.views)
    .slice(0, 10);

  // Views pa jou (28 dènye jou)
  ...
  const perDay = Object.entries(dayMap).map(([date, views]) => ({ date, views }));

  // Jou pi bon (max views)
  const bestDay = perDay.reduce((best, d) => d.views > (best?.views || 0) ? d : best, null);
  ...
  res.json({ totalViews: totalViews || 0, views28: views28 || 0, topListings, perDay, bestDay, activeListings: activeListings || 0 });
});
