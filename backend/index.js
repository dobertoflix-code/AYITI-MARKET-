...
app.post('/api/upload-image', uploadJsonParser, async (req, res) => {
  ...
  const safeFolder = ['avatars', 'broadcast', 'listings', 'business', 'ads'].includes(folder) ? folder : 'misc';
...
// ════════════════════════════════════════════════════
// PAYPAL — pou moun ki vle peye an dola (USD)
// PayPal pa aksepte HTG, donk nou konvèti pri HTG an USD
// avèk yon to chanj fiks configire nan .env (PAYPAL_HTG_PER_USD).
// ════════════════════════════════════════════════════
const PAYPAL_MODE = process.env.PAYPAL_MODE === 'live' ? 'live' : 'sandbox';
const PAYPAL_BASE = PAYPAL_MODE === 'live'
  ? 'https://api-m.paypal.com'
  : 'https://api-m.sandbox.paypal.com';
const PAYPAL_HTG_PER_USD = Number(process.env.PAYPAL_HTG_PER_USD) || 132;

function htgToUsd(priceHtg) {
  const usd = Number(priceHtg) / PAYPAL_HTG_PER_USD;
  return Math.max(1, Math.round(usd * 100) / 100); // minimòm 1 USD
}
...
async function getPaypalToken() {
  ...
  if (!r.ok) throw new Error('Pa kapab konekte ak PayPal (verifye kle API yo)');
  const data = await r.json();
  return data.access_token;
}

// Kreye yon lòd PayPal (Orders API v2) epi retounen lyen apwobasyon an
async function createPaypalOrder({ priceHtg, referenceId, returnPath, description }) {
  const accessToken = await getPaypalToken();
  const frontendUrl = (process.env.FRONTEND_URL || '').replace(/\/$/, '');
  const usdAmount = htgToUsd(priceHtg).toFixed(2);
  ...
  const r = await fetch(`${PAYPAL_BASE}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      ...
      application_context: {
        brand_name: 'Ayiti Market',
        user_action: 'PAY_NOW',
        return_url: `${process.env.BACKEND_URL || ''}${returnPath}`,
        cancel_url: `${frontendUrl}/account.html?paypal=cancelled`
      }
    })
  });
  const data = await r.json();
  if (!r.ok) throw new Error(data?.message || 'PayPal pa kapab kreye lòd la');
  const approveLink = data.links?.find(l => l.rel === 'approve')?.href;
  if (!approveLink) throw new Error('PayPal pa retounen yon lyen apwobasyon');
  return { orderId: data.id, approveLink, usdAmount };
}

// Kaptire (finalize) yon peman PayPal aprè itilizatè a apwouve l
async function capturePaypalOrder(orderId) {
  ...
  const data = await r.json();
  if (!r.ok) throw new Error(data?.message || 'PayPal pa kapab finalize peman an');
  const capture = data?.purchase_units?.[0]?.payments?.captures?.[0];
  if (!capture || capture.status !== 'COMPLETED') throw new Error('Peman PayPal la pa konplete');
  return {
    amountUsd: Number(capture.amount?.value || 0),
    transactionId: capture.id,
    referenceId: data?.purchase_units?.[0]?.reference_id || null
  };
}
...
app.post('/api/boost/initiate', async (req, res) => {
  ...
  if (!['moncash', 'natcash', 'paypal'].includes(method)) return res.status(400).json({ error: 'Metòd peman envalid' });
  ...
  if (method === 'paypal') {
    try {
      const orderId = `boost-${payment.id}`;
      const { orderId: paypalOrderId, approveLink } = await createPaypalOrder({
        priceHtg: price,
        referenceId: orderId,
        returnPath: `/api/boost/paypal/return?ref=${orderId}`,
        description: `Boost anons: ${listing.title}`
      });

      await supabase.from('boost_payments').update({ paypal_order_id: paypalOrderId }).eq('id', payment.id);
      ...
      await supabase.from('boost_payments').update({ status: 'failed' }).eq('id', payment.id);
      return res.status(502).json({ error: 'Erè PayPal: ' + err.message });
    }
  }
...
// ── GET /api/boost/paypal/return ───────────────────
// PayPal redireksyone isit aprè itilizatè a apwouve peman an
app.get('/api/boost/paypal/return', async (req, res) => {
  const frontendUrl = (process.env.FRONTEND_URL || '').replace(/\/$/, '');
  const { token, ref } = req.query; // 'token' se orderId PayPal la

  try {
    if (!token) throw new Error('PayPal pa retounen okenn referans lòd');
    const { amountUsd, transactionId, referenceId } = await capturePaypalOrder(token);

    // Sekirite: itilize referans ke NOU te kreye a (ref, ki soti nan return_url
    // konfigire pa nou), pa yon valè ki ta ka manipile nan URL la san verifikasyon.
    const safeRef = referenceId || ref;
    if (!safeRef || !safeRef.startsWith('boost-')) throw new Error('Referans peman envalid');

    const paymentId = safeRef.replace('boost-', '');
    ...
    // Sekirite: dosye peman an dwe deja gen menm orderId PayPal ki te kreye l la
    if (payment.paypal_order_id && payment.paypal_order_id !== token) {
      throw new Error('Dosye peman pa koresponn ak lòd PayPal la');
    }

    // Sekirite: konfime montan peye a (konvèti an USD) egal ak sa ki te dwe peye a.
    const expectedUsd = htgToUsd(payment.price_htg);
    if (Math.abs(amountUsd - expectedUsd) > 0.02) {
    ...
    if (payment.status !== 'paid') {
      await applyBoost({
        ...
        notes: `PayPal #${transactionId}`, method: 'paypal'
      });
  ...
  if (method === 'paypal') {
    try {
      ...
      const { orderId: paypalOrderId, approveLink } = await createPaypalOrder({
        priceHtg: price,
        referenceId: orderId,
        returnPath: `/api/shop-pro/paypal/return?ref=${orderId}`,
        description: 'Abònman Kont Boutik Pro — 30 jou'
      });

      await supabase.from('shop_pro_payments').update({ paypal_order_id: paypalOrderId }).eq('id', payment.id);
...
// ── GET /api/shop-pro/paypal/return ─────────────────
// PayPal redireksyone isit aprè itilizatè a apwouve peman an
app.get('/api/shop-pro/paypal/return', async (req, res) => {
  const frontendUrl = (process.env.FRONTEND_URL || '').replace(/\/$/, '');
  ...
  try {
    ...
    const safeRef = referenceId || ref;
    if (!safeRef || !safeRef.startsWith('pro-')) throw new Error('Referans peman envalid');

    const paymentId = safeRef.replace('pro-', '');
    ...
    const expectedUsd = htgToUsd(payment.price_htg);
    if (Math.abs(amountUsd - expectedUsd) > 0.02) {
      throw new Error('Montan peye a pa egal ak pri abònman an');
    }
...
// ════════════════════════════════════════════════════
// KONT ANTREPRIZ (Business Accounts)
// ════════════════════════════════════════════════════

const BUSINESS_CATEGORIES = [
  'Restoran & Bwason', 'Otèl & Ejans Vwayaj', 'Sante & Bote', 'Edikasyon',
  'Elektwonik & Teknoloji', 'Machin & Transpò', 'Iminobilye', 'Mòd & Akseswa',
  'Sèvis Finansye', 'Konstriksyon & BTP', 'Agrikilti', 'Evènman & Divètisman', 'Lòt'
];

// ── POST /api/business/register ────────────────────
// Rele imedyatman apre auth.signUp() reisi (oswa apre premye koneksyon si
// imèl konfimasyon te aktive) pou transfòme yon kont an "kont Antrepriz".
// Body: { business_name, business_category, business_address, business_phone, location }
app.post('/api/business/register', async (req, res) => {
  const user = await requireAuthUser(req, res);
  if (!user) return;

  const { business_name, business_category, business_address, business_phone, location } = req.body || {};
  if (!business_name || !business_name.trim()) {
    return res.status(400).json({ error: 'Non biznis la obligatwa' });
  }
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      full_name:           business_name.trim(),
      business_category:   business_category || null,
      phone:                business_phone || null,
      location:             business_address || location || null
    })
    .eq('id', user.id)
    .select('id, account_type, business_name')
    .single();
...
// Antrepriz la modifye pwòp enfo li (deskripsyon, orè, sit wèb, logo/cover).
app.put('/api/business/profile', async (req, res) => {
  const user = await requireAuthUser(req, res);
  if (!user) return;

  const { data: existing } = await supabase.from('profiles').select('account_type').eq('id', user.id).single();
  if (!existing || existing.account_type !== 'business') {
    return res.status(403).json({ error: 'Sa a se yon fonksyon rezève pou kont Antrepriz' });
  }

  const allowed = ['business_name', 'business_category', 'business_hours',
    'website', 'cover_url', 'avatar_url', 'bio', 'location', 'phone'];
  const updates = {};
  for (const key of allowed) {
    if (req.body[key] !== undefined) updates[key] = req.body[key];
  }
  if (updates.business_name) updates.full_name = updates.business_name;
  if (Object.keys(updates).length === 0) return res.status(400).json({ error: 'Anyen pou modifye' });
...
// ── PUT /api/admin/profiles/:id/business-verify ────
app.put('/api/admin/profiles/:id/business-verify', async (req, res) => {
  const user = await requireAdmin(req, res);
  if (!user) return;
  const { business_verified } = req.body;
  const { data, error } = await supabase
    .from('profiles').update({ business_verified: !!business_verified })
    .eq('id', req.params.id).select('id, business_verified').single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

// ════════════════════════════════════════════════════
// ESPAS PIBLISITE (Ad Banners) — jere manyèlman pa admin
// ════════════════════════════════════════════════════

// ── GET /api/ads ────────────────────────────────────
// Piblik. Query: ?position=akey_anwo|akey_mitan|sidebar|paj_anons
app.get('/api/ads', async (req, res) => {
  const { position } = req.query;
  let q = supabase.from('ad_banners').select('id, title, image_url, link_url, position, sort_order')
...
// Konte klik (pa bloke — toujou reponn success menm si echwe).
app.post('/api/ads/:id/click', async (req, res) => {
  supabase.from('ad_banners').select('clicks').eq('id', req.params.id).single()
    .then(({ data }) => {
      if (data) supabase.from('ad_banners').update({ clicks: (data.clicks || 0) + 1 }).eq('id', req.params.id).then(() => {});
    })
    .catch(() => {});
  res.json({ success: true });
});
...
// ── POST /api/admin/ads ─────────────────────────────
// Body: { title, image_url, link_url, position, sort_order }
app.post('/api/admin/ads', async (req, res) => {
  const user = await requireAdmin(req, res);
  if (!user) return;
  const { title, image_url, link_url, position, sort_order } = req.body || {};
  if (!title || !image_url || !link_url) {
    return res.status(400).json({ error: 'Title, image_url ak link_url obligatwa' });
  }
  const { data, error } = await supabase.from('ad_banners').insert({
    title, image_url, link_url,
    position: position || 'akey_anwo',
...
// Body: nenpòt konbinezon nan { title, image_url, link_url, position, sort_order, is_active }
app.put('/api/admin/ads/:id', async (req, res) => {
  const user = await requireAdmin(req, res);
  if (!user) return;
  const allowed = ['title', 'image_url', 'link_url', 'position', 'sort_order', 'is_active'];
  const updates = {};
  for (const key of allowed) {
    if (req.body[key] !== undefined) updates[key] = req.body[key];
  }
  if (Object.keys(updates).length === 0) return res.status(400).json({ error: 'Anyen pou modifye' });
  const { data, error } = await supabase.from('ad_banners').update(updates).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
app.delete('/api/admin/ads/:id', async (req, res) => {
  const user = await requireAdmin(req, res);
  if (!user) return;
  const { error } = await supabase.from('ad_banners').delete().eq('id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true });
});
...
// ── SEO/OG: paj sèvè pou pwofil vandè (/boutik/:id/:slug) ─
// Sèl bi a se bay yon bèl previzyalizasyon (Open Graph) lè yon moun pataje
// lyen boutik li sou WhatsApp/Facebook — kontni reyèl la rete sou seller.html.
app.get('/boutik/:id/:slug?', async (req, res) => {
  const { id } = req.params;
  const frontendUrl = (process.env.FRONTEND_URL || 'https://ayiti-market.com').replace(/\/$/, '');

  const { data: profile, error } = await supabase
    .from('profiles')
    .select('full_name, location, avatar_url, is_verified_seller, pro_seller_until, bio')
    .eq('id', id)
    .single();

  if (error || !profile) {
    return res.status(404).send(`<!DOCTYPE html><html lang="ht"><head><meta charset="utf-8">
      <title>Boutik pa jwenn — Ayiti Market</title>
      <meta name="robots" content="noindex">
      <meta http-equiv="refresh" content="2;url=${frontendUrl}/">
      </head><body>Boutik sa a pa egziste ankò. Ou pral retounen sou Ayiti Market...</body></html>`);
  }
  ...
  const isPro = profile.pro_seller_until && new Date(profile.pro_seller_until) > new Date();
  const badges = [profile.is_verified_seller ? 'Vandè Verifye ✓' : null, isPro ? 'Boutik Pro 👑' : null]
    .filter(Boolean).join(' · ');

  const sellerName = profile.full_name || 'Vandè';
  const correctSlug = slugify(sellerName);
...
<meta name="twitter:image" content="${escapeHtml(image)}">
<style>
  body{font-family:system-ui,-apple-system,sans-serif;max-width:680px;margin:0 auto;padding:24px;color:#1a1a2e;line-height:1.6;text-align:center}
  img{width:96px;height:96px;border-radius:50%;object-fit:cover;background:#eee}
  .badges{color:#0a8;font-weight:600;font-size:0.92rem;margin:6px 0 18px}
  .btn{display:inline-block;background:#1a1a2e;color:#fff;padding:14px 26px;border-radius:10px;text-decoration:none;font-weight:600;margin-top:10px}
</style>
</head>
<body>
  <img src="${escapeHtml(image)}" alt="${escapeHtml(sellerName)}">
  <h1>${escapeHtml(sellerName)}</h1>
  ${badges ? `<div class="badges">${escapeHtml(badges)}</div>` : ''}
  <p>${escapeHtml(description)}</p>
  <a class="btn" href="${appUrl}">Wè boutik la sou Ayiti Market →</a>
</body>
</html>`);
});

// ── SEO/OG: paj sèvè pou pwofil Antrepriz (/biznis/:id/:slug) ─
// Menm lojik ak /boutik, men pou kont "business" — bay bèl previzyalizasyon
// (Open Graph) lè yon moun pataje lyen paj biznis la. Kontni reyèl la rete
// sou seller.html (li deja jere paj biznis kounye a).
app.get('/biznis/:id/:slug?', async (req, res) => {
  const { id } = req.params;
  const frontendUrl = (process.env.FRONTEND_URL || 'https://ayiti-market.com').replace(/\/$/, '');

  const { data: profile, error } = await supabase
    .from('profiles')
    .select('business_name, business_category, location, avatar_url, cover_url, business_verified, bio, account_type')
    .eq('id', id).eq('account_type', 'business')
    .single();

  if (error || !profile) {
    return res.status(404).send(`<!DOCTYPE html><html lang="ht"><head><meta charset="utf-8">
      <title>Antrepriz pa jwenn — Ayiti Market</title>
      <meta name="robots" content="noindex">
      <meta http-equiv="refresh" content="2;url=${frontendUrl}/">
      </head><body>Paj biznis sa a pa egziste ankò. Ou pral retounen sou Ayiti Market...</body></html>`);
  }
  ...
  const badges = [profile.business_verified ? 'Antrepriz Verifye ✓' : null, profile.business_category].filter(Boolean).join(' · ');
  const bizName = profile.business_name || 'Antrepriz';
  const correctSlug = slugify(bizName);
  const title = `${bizName} — Paj Biznis sou Ayiti Market`;
  const description = (profile.bio && profile.bio.trim())
    || `${bizName} gen ${listingCount || 0} anons aktif sou Ayiti Market${profile.location ? ` nan ${profile.location}` : ''}.${badges ? ' ' + badges : ''}`;
  const image = profile.cover_url || profile.avatar_url || `${frontendUrl}/og-image.png`;
  const canonicalUrl = `${frontendUrl}/biznis/${id}/${correctSlug}`;
  const appUrl = `${frontendUrl}/seller.html?id=${id}`;
...
<style>
  body{font-family:system-ui,-apple-system,sans-serif;max-width:680px;margin:0 auto;padding:24px;color:#1a1a2e;line-height:1.6;text-align:center}
  img{width:96px;height:96px;border-radius:14px;object-fit:cover;background:#eee}
  .badges{color:#0a8;font-weight:600;font-size:0.92rem;margin:6px 0 18px}
  .btn{display:inline-block;background:#1a1a2e;color:#fff;padding:14px 26px;border-radius:10px;text-decoration:none;font-weight:600;margin-top:10px}
</style>
</head>
<body>
  <img src="${escapeHtml(image)}" alt="${escapeHtml(bizName)}">
  <h1>${escapeHtml(bizName)}</h1>
  ${badges ? `<div class="badges">${escapeHtml(badges)}</div>` : ''}
  <p>${escapeHtml(description)}</p>
  <a class="btn" href="${appUrl}">Wè paj biznis la sou Ayiti Market →</a>
</body>
</html>`);
});
