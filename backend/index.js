...
// ══════════════════════════════════════════════════
// SISTÈM NÒT VANDÈ — Reviews
// ══════════════════════════════════════════════════

// GET /api/reviews/:sellerId — Tout nòt pou yon vandè
app.get('/api/reviews/:sellerId', async (req, res) => {
  const { sellerId } = req.params;
  const { data, error } = await supabase
    .from('reviews')
    .select('id, rating, comment, created_at, reviewer_id, listing_id, profiles:reviewer_id(full_name, username)')
    .eq('seller_id', sellerId)
    .order('created_at', { ascending: false })
    .limit(50);
  ...
  // Ajoute reviewer_name nan chak review
  const reviews = (data || []).map(r => ({
    ...r,
    reviewer_name: r.profiles?.full_name || r.profiles?.username || 'Itilizatè'
  }));

  res.json(reviews);
});

// POST /api/reviews — Kite yon nòt
app.post('/api/reviews', async (req, res) => {
  ...
  if (!seller_id || !rating || rating < 1 || rating > 5) {
    return res.status(400).json({ error: 'Done invalide — rating 1-5 obligatwa' });
  }

  if (seller_id === user.id) {
    return res.status(400).json({ error: 'Ou pa ka kite nòt pou pwòp tèt ou' });
  }
  ...
  const review = {
    ...
    comment: (comment || '').trim().slice(0, 400) || null
  };

  const { data, error } = await supabase
    .from('reviews')
    .upsert(review, { onConflict: 'seller_id,reviewer_id' })
    .select()
    .single();
...
// DELETE /api/reviews/:reviewId — Efase pwòp nòt ou
