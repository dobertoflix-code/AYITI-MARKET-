...
// ── POST /api/reports ──────────────────────────────
// Nenpòt moun ka soumèt yon rapò (pa bezwen konekte)
app.post('/api/reports', async (req, res) => {
  const { listing_id, reason, details, reporter_email } = req.body;
  if (!reason) return res.status(400).json({ error: 'Rezon obligatwa' });

  // Jwenn reporter_id si itilizatè a konekte
  ...
  const { data, error } = await supabase
    .from('reports')
    .insert({ listing_id: listing_id || null, reason, details: details || null, reporter_email: reporter_email || null, reporter_id })
    .select()
    .single();

  if (error) return res.status(500).json({ error: error.message });

  // Si menm anons lan gen 3 rapò oswa plis, mete li 'flagged' otomatikman
  if (listing_id) {
    ...
    if (count >= 3) {
      await supabase
        ...
        .eq('id', listing_id)
        .eq('status', 'active'); // Pa ekrase si déjà retire
    }
  }
...
// Lis tout rapò (admin sèlman), filtre pa status
app.get('/api/admin/reports', async (req, res) => {
  ...
  let query = supabase
    ...
    .select(`
      *,
      listing:listings(id, title, status, images, user_id),
...
// Admin: rezoud yon rapò — action: 'dismiss' | 'remove_listing' | 'warn_user'
app.put('/api/admin/reports/:id', async (req, res) => {
  const user = await requireAdmin(req, res);
  if (!user) return;

  const { action, note } = req.body;
  if (!['dismiss', 'remove_listing', 'warn_user'].includes(action)) {
  ...
  if (!report) return res.status(404).json({ error: 'Rapò pa jwenn' });

  // Aplike aksyon an
  if (action === 'remove_listing' && report.listing_id) {
    await supabase.from('listings').update({ status: 'removed' }).eq('id', report.listing_id);
    // Fèmen tout lòt rapò ki gen menm anons lan
    await supabase.from('reports')
      .update({ status: 'resolved', resolved_by: user.id, resolved_at: new Date().toISOString(), action_taken: action, admin_note: note || null })
...
// ── GET /api/admin/stats ───────────────────────────
// Estatistik jeneral pou dashboard admin
app.get('/api/admin/stats', async (req, res) => {
  ...
  const [
    ...
    supabase.from('listings').select('*', { count: 'exact', head: true }).eq('status', 'flagged'),
    supabase.from('reports').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
    supabase.from('profiles').select('*', { count: 'exact', head: true }),
  ]);

  res.json({ totalListings, activeListings, flaggedListings, pendingReports, totalUsers });
});
